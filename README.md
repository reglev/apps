# Leo's Spiele-Sammlung

## 🔗 Live spielen

**GitHub Pages:** https://reglev.github.io/apps/

### Flucht im Dunkeln - Leo's Survival Horror Game

Ein Top-Down Horror-Spiel mit Taschenlampe und gruseligen Celestial-Monster.

**[Direkt zum Spiel](https://reglev.github.io/apps/game/blutspuren.html)**

## Steuerung
- **WASD** oder **Pfeiltasten** zum Bewegen
- Taschenlampe zeigt in Laufrichtung

## Spielziel
Finde alle Blutspuren um zu entkommen!

## Monster Fähigkeiten

### Divine Prediction
- Teleportiert alle 8 Sekunden vor den Spieler
- Roter Flash-Effekt beim Teleport

### Lights Out
- Alle 13 Sekunden
- 3 Sekunden Flackern → 2 Sekunden komplette Dunkelheit

### Dark Pulse Realm
- Alle 20 Sekunden für 4 Sekunden aktiv
- Monster wird 2x schneller
- Spieler wird 30% langsamer
- Lila Screen-Effekt

## Version
1.0 - AI Entwickelt mit Leo🦎
